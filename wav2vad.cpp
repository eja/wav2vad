    ...
    void predict(const std::vector<float> &data)
    {
        ...
        if ((speech_prob >= (threshold - 0.15)) && (speech_prob < threshold))
        {
           if (triggered) {
                ...
                printf(" { \"time\": %.3f, \"confidence\": %.3f }", 1.0 * speech / sample_rate, speech_prob);
					 }
            return;
        }
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <ONNX_model_file> <WAV_file>" << std::endl;
        return 1;
    }
    ...
    // Open the WAV file using sndfile
    SF_INFO sf_info;
    SNDFILE* sndfile = sf_open(wav_file.c_str(), SFM_READ, &sf_info);
    if (!sndfile) {
